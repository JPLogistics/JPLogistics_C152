import { Units, NavMath, UnitType, GeoPoint } from 'msfssdk';
import { EventBus, ControlEvents, Consumer } from 'msfssdk/data';
import { Localizer, CdiDeviation, Radio, RadioType, NavSourceId, NavSourceType, RadioEvents, FrequencyBank, ADCEvents, NavEvents, GNSSEvents } from 'msfssdk/instruments';
...
import { LNavData, LNavEvents } from './Directors/LNavDirector';
import { FlightPlanner } from 'msfssdk/flightplan';

/**
 * A G1000 NXi nav-to-nav manager.
 */
export class G1000NavToNavManager implements NavToNavManager {
   ...
   */
  private tryAutoSwitchSource(d: LNavData): void {
    const plan = this.flightPlanner.hasActiveFlightPlan() && this.flightPlanner.getActiveFlightPlan();
    const currentLeg = plan && d.currentLegIndex >= 0 && d.currentLegIndex < plan.length ? plan.getLeg(d.currentLegIndex) : undefined;
  ...
  private monitorEvents(): void {
    const sub = this.bus.getSubscriber<RadioEvents & NavEventsExtentions & ADCEvents & GNSSEvents & LNavEvents & G1000ControlEvents>();

    sub.on('setRadioState').handle((state) => {
      this.updateRadioState(state);
    });
    sub.on('setFrequency').handle((frequency) => {
    ...
    sub.on('nav_1_localizer').handle((loc) => {
      this.nav1Localizer = loc;
      this.updateState();
    });
    sub.on('nav_2_localizer').handle((loc) => {
      this.nav2Localizer = loc;
      this.updateState();
    });
    sub.on('nav_1_cdi').handle((cdi) => {
      this.nav1Cdi = cdi;
    });
    sub.on('nav_2_cdi').handle((cdi) => {
      this.nav2Cdi = cdi;
    });
