...
import {
  ...
  FacilityRespository, VisualFacility, ExtendedApproachType, RnavTypeFlags, AdditionalApproachType, DepartureProcedure, ArrivalProcedure
...
import { Autopilot } from 'msfssdk/autopilot';
...
export class Fms {
  ...
  public static g1000EvtPub: G1000ControlPublisher;

  public readonly ppos = new GeoPoint(0, 0);

  private readonly facRepo = FacilityRespository.getRepository(this.bus);
  ...
  constructor(
    ...
    g1000EvtPub: G1000ControlPublisher,
    ...
    const g1000Events = this.bus.getSubscriber<G1000ControlEvents>();
  ...
  private moveDirectToDestinationLeg(plan: FlightPlan, targetSegmentType: FlightPlanSegmentType, arrivalSegmentIndex?: number): boolean {
    if (this.getDirectToState() === DirectToState.TOEXISTING) {
      ...
      if (directTargetSegmentIndex !== undefined && directTargetSegmentIndex > 0 && plan.getLeg(plan.activeLateralLeg).leg.fixIcao === plan.destinationAirport) {
        ...
        plan.setCalculatingLeg(plan.activeLateralLeg);
  ...
  public activateLeg(segmentIndex: number, legIndex: number, fplnIndex = 0, inhibitImmediateSequence = false): void {
    ...
    if (
      (fplnIndex !== 0 && this.getDirectToState() === DirectToState.TOEXISTING)
    ...
    if (inhibitImmediateSequence) {
      Fms.g1000EvtPub.publishEvent('lnav_inhibit_next_sequence', true);
    }
  }
   ...
   * @param course The course for the Direct To. If undefined, the Direct To will be initiated from the airplane's
   * present position.
   */
  ...
  public createDirectToRandom(target: string | Facility, course?: number): void {
    ...
    if (segment) {
      ...
      if (course) {
        originPos = this.ppos.offset(NavMath.normalizeHeading(course + 180), UnitType.NMILE.convertTo(500, UnitType.GA_RADIAN));
      }
      const discoLeg = FlightPlan.createLeg({ type: LegType.Discontinuity });
      const dtoOriginLeg = this.createDTOOriginLeg(originPos);
      const dtoTargetLeg = this.createDTODirectLeg(icao);
   ...
   */
  private createDTODirectLeg(icao: string, leg?: FlightPlanLeg): FlightPlanLeg {
  ...
  public async buildProcedurePreviewPlan(
    ...
    const plan = new FlightPlan(0, calculator);
  ...
  private planRemoveLeg(segmentIndex: number, segmentLegIndex: number, notify = true, skipDupCheck = false, skipCancelDirectTo = false): boolean {
    ...
    if (airwayLegType !== AirwayLegType.NONE) {
      ...
      removed = plan.removeLeg(segmentIndex, segmentLegIndex, notify) !== null ? true : false;
    }
   ...
   */
  private removeLegAirwayHandler(plan: FlightPlan, airwayLegType: AirwayLegType, segmentIndex: number, index: number): boolean {
    ...
    if (toDeleteIndex >= 0) {
      ...
      if (needMoveDtoLegs) {
        ...
        if (isDtoExistingActive) {
          plan.setLateralLeg(preservedLegIndex + FmsUtils.DTO_LEG_OFFSET);
        }
      }
   ...
   */
  private setLocFrequency(radioIndex: number, forceNotify = false): void {
