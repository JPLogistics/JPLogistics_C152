...
import { MapFlightPlanHoldLegPathRenderer, MapFlightPlanLegLineRenderer, MapFlightPlanObsLegPathRenderer, MapFlightPlanVtfLegPathRenderer } from './MapFlightPlanLegPathRenderers';
...
export class MapDefaultBaseRouteFlightPlanPathRenderer extends MapAbstractFlightPlanPathRenderer {
  ...
  protected renderLeg(
    ...
    if (!BitFlags.isAll(leg.flags, LegDefinitionFlags.DirectTo) || legIndex === activeLegIndex) {
      if (BitFlags.isAny(leg.flags, LegDefinitionFlags.VectorsToFinal) && leg.leg.type === LegType.CF) {
