...
import { GeoCircle, GeoPoint, NavMath, UnitType, Vec3Math } from '..';
...
export class JoinGreatCircleToPointBuilder {
    ...
    build(vectors, index, start, startPath, end, endPath, desiredTurnDirection, minTurnRadius, preferSingleTurn = false, intersection, flags = 0, includeTurnToCourseFlag = true) {
        ...
        if (needCalculateTwoTurnPath) {
            ...
            const endTurnDirection = ((towardEndPointTurnDirection === 'left')
            ...
            if (Math.abs(endTurnCircle.distance(startTurnCenter)) < minTurnRadiusRad - GeoPoint.EQUALITY_TOLERANCE && desiredTurnDirection !== endTurnDirection) {
                ...
                const endTurnOffsetCircle = JoinGreatCircleToPointBuilder.geoCircleCache[1].set(FlightPathUtils.getTurnCenterFromCircle(endTurnCircle, JoinGreatCircleToPointBuilder.vec3Cache[5]), minTurnRadiusRad * 2);
                const startPathOffsetCircle = JoinGreatCircleToPointBuilder.geoCircleCache[2].set(startPath.center, startPath.radius + minTurnRadiusRad * (desiredTurnDirection === 'left' ? -1 : 1));
                const intersections = JoinGreatCircleToPointBuilder.intersectionCache;
