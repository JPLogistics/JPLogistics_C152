import { BitFlags, GeoCircle, GeoPoint, MagVar, NavMath, UnitType, Vec3Math } from '..';
import { FacilityType, ICAO, LegTurnDirection, LegType } from '../navigation';
...
export class DirectToFixLegCalculator extends AbstractFlightPathLegCalculator {
    ...
    calculateVectors(legs, calculateIndex, activeLegIndex, state) {
        ...
        if (!turnEndPoint.equals(startPoint)) {
            vectorIndex += this.circleVectorBuilder.build(vectors, vectorIndex, turnDirection, UnitType.GA_RADIAN.convertTo(turnRadiusRad, UnitType.METER), turnCenterPoint, startPoint, turnEndPoint, FlightPathVectorFlags.TurnToCourse);
        }
    ...
    calculateVectors(legs, calculateIndex, activeLegIndex, state) {
        ...
        if (isDirectEntry) {
            ...
            if (directionalEntryCourse > 135) {
                ...
                ingressVectorIndex += this.turnToCourseBuilder.build(ingress, ingressVectorIndex, holdPos, turnRadiusMeters, turnDirection === 'left' ? 'right' : 'left', state.currentCourse, outboundCourse, FlightPathVectorFlags.HoldEntry | FlightPathVectorFlags.TurnToCourse);
            ...
            ingressVectorIndex += this.joinGreatCircleToPointBuilder.build(ingress, ingressVectorIndex, state.currentPosition, this.geoCircleCache[1].setAsGreatCircle(state.currentPosition, state.currentCourse), holdPos, inboundPath, turnDirection, turnRadiusMeters, true, undefined, FlightPathVectorFlags.HoldEntry);
            if (skipRacetrack) {
            ...
            ingressVectorIndex += this.turnToCourseBuilder.build(ingress, ingressVectorIndex, holdPos, turnRadiusMeters, turnDirection === 'left' ? 'right' : 'left', state.currentCourse, parallelCourse, FlightPathVectorFlags.HoldEntry | FlightPathVectorFlags.TurnToCourse);
            const turnVector = ingress[ingressVectorIndex - 1];
            state.currentPosition.set(turnVector.endLat, turnVector.endLon);
            state.currentCourse = (_f = FlightPathUtils.getVectorFinalCourse(turnVector)) !== null && _f !== void 0 ? _f : parallelCourse;
            ingressVectorIndex += this.procTurnBuilder.build(ingress, ingressVectorIndex, state.currentPosition, this.geoCircleCache[1].setAsGreatCircle(state.currentPosition, state.currentCourse), holdPos, inboundPath, course + 135 * turnDirectionSign, turnRadiusMeters, turnDirection === 'left' ? 'right' : 'left', state.currentCourse, course, FlightPathVectorFlags.HoldEntry);
            calcs.ingressJoinIndex = 0;
        }
