 ...
 */
export class MapAbstractFlightPlanPathRenderer {
    /**
     * Constructor.
     ...
     */
    render(plan, startIndex, endIndex, projection, context, stream, ...args) {
        ...
        for (const leg of plan.legs(isReverse, startIndex)) {
            ...
            this.renderLeg(leg, plan, activeLeg, index, activeLegIndex, projection, context, stream, ...args);
            index += delta;
        }
        if (this.renderActiveLegLast && activeLeg) {
            this.renderLeg(activeLeg, plan, activeLeg, activeLegIndex, activeLegIndex, projection, context, stream, ...args);
        }
    }
}
