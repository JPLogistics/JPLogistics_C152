import { LegDefinitionFlags, FlightPlanSegmentType } from '../flightplan';
 ...
 */
export class SmoothingPathCalculator {
    /**
     ...
     */
    buildVerticalLegsAndConstraints(lateralPlan, verticalPlan, verticalDirectIndex) {
        var _a, _b, _c, _d, _e;
        ...
        for (const segment of lateralPlan.segments()) {
            ...
            for (let legIndex = 0; legIndex < segment.legs.length; legIndex++) {
                ...
                const verticalLeg = VNavUtils.createLeg(segment.segmentIndex, legIndex, (_a = lateralLeg.name) !== null && _a !== void 0 ? _a : '', (_c = (_b = lateralLeg.calculated) === null || _b === void 0 ? void 0 : _b.distanceWithTransitions) !== null && _c !== void 0 ? _c : undefined);
                ...
                if (constraintAltitudes !== undefined && !legPrecedesVerticalDirectIndex && !legPrecedesDirectTo) {
                    const verticalConstraint = VNavUtils.createConstraint(globalLegIndex, constraintAltitudes[0], constraintAltitudes[1], verticalLeg.name, legIsInDepartureOrMissed ? 'climb' : 'descent');
     ...
     * @param verticalDirectIndex The vertical direct index, if any.
     */
    buildVerticalFlightPlan(lateralPlan, verticalPlan, verticalDirectIndex) {
     ...
     */
    computeVnavPath(lateralPlan, verticalPlan) {
     ...
     * @param lateralPlan The Lateral Flight Plan.
     */
    checkInvalidConstrantsAndReinsertIfValid(verticalPlan, lateralPlan) {
                ...
                if (verticalLeg.invalidConstraintAltitude !== undefined) {
                    ...
                    if (lateralLeg !== null) {
                        ...
                        if (constraintAltitudes !== undefined) {
                            ...
                            if (!SmoothingPathCalculator.isConstraintHigherThanPriorConstraint(previousConstraint, proposedConstraint) &&
     ...
     */
    computeFlightPathAngles(verticalPlan, lateralPlan) {
        ...
        for (let targetConstraintIndex = lastDescentConstraintIndex; targetConstraintIndex <= firstDescentConstraintIndex; targetConstraintIndex++) {
            ...
            if (currentTargetConstraint !== undefined && firstDescentConstraintIndex !== undefined) {
                ...
                if (currentTargetConstraintIsFirstDescentConstraint) {
                    currentTargetConstraint = this.handleDirectToTargetConstraint(currentTargetConstraint, currentPathSegmentMinFpa, currentPathSegmentMaxFpa, verticalPlan, lateralPlan);
                ...
                for (let currentConstraintIndex = targetConstraintIndex + 1; currentConstraintIndex <= firstDescentConstraintIndex; currentConstraintIndex++) {
                    ...
                    if (minFpa > currentPathSegmentMaxFpa || maxFpa < currentPathSegmentMinFpa || currentConstraintIsFirstDescentConstraint || currentConstraintIsFaf) {
                        currentTargetConstraint = this.calculateAndSetTargetConstraintFpa(currentTargetConstraint, minFpa, maxFpa, currentPathSegmentMinFpa, currentPathSegmentMaxFpa, currentConstraintIsFirstDescentConstraint, currentConstraintIsFaf);
                        ...
                        const applyPathValuesResults = SmoothingPathCalculator.applyPathValuesToSmoothedConstraints(verticalPlan, targetConstraintIndex, currentConstraintIndex, nextMaxAltitude);
                        ...
                        const proposedNextTargetConstraintAltitude = currentTargetConstraint.targetAltitude + VNavUtils.altitudeForDistance(currentTargetConstraint.fpa, smoothedSegmentDistance);
                        ...
                        currentTargetConstraint.targetAltitude = MathUtils.clamp(proposedNextTargetConstraintAltitude, currentTargetConstraint.minAltitude, Math.min(currentTargetConstraint.maxAltitude, nextMaxAltitude));
     ...
     */
    handleDirectToTargetConstraint(targetConstraint, currentPathSegmentMinFpa, currentPathSegmentMaxFpa, verticalPlan, lateralPlan) {
     ...
     * @returns The index or undefined.
     */
    calculateAndSetTargetConstraintFpa(targetConstraint, legMinFpa, legMaxFpa, currentPathSegmentMinFpa, currentPathSegmentMaxFpa, currentConstraintIsFirstDescentConstraint, currentConstraintIsFaf) {
     ...
     */
    static getConstraintAltitudes(leg) {
     ...
     */
    static setFirstApproachConstraintAltitudes(verticalPlan, approachStartGlobalLegIndex) {
     ...
     */
    static getDirectToGlobalLegIndex(lateralPlan) {
     ...
     * @param verticalPlan The Vertical Flight Plan.
     */
    static setDirectToLegInVerticalPlan(lateralPlan, verticalPlan) {
        ...
        if (lateralPlan.directToData.segmentIndex > -1 && lateralPlan.directToData.segmentLegIndex > -1) {
            ...
            if (BitFlags.isAll(directLateralLeg.flags, LegDefinitionFlags.DirectTo)) {
                const directVerticalLeg = VNavUtils.getVerticalLegFromSegmentInPlan(verticalPlan, lateralPlan.directToData.segmentIndex, lateralPlan.directToData.segmentLegIndex + lateralPlanDirectToOffset);
     ...
     */
    static isConstraintInDepartureOrMissed(lateralSegment, lateralLeg) {
     ...
     * @param verticalPlan The Vertical Flight Plan.
     */
    static fillLegDistances(lateralPlan, verticalPlan) {
        var _a, _b, _c;
     ...
     * @param verticalPlan The Vertical Flight Plan.
     */
    static populateConstraints(verticalPlan) {
        for (let constraintIndex = 0; constraintIndex < verticalPlan.constraints.length; constraintIndex++) {
            const constraint = verticalPlan.constraints[constraintIndex];
     ...
     */
    static findPriorMaxAltitude(verticalPlan, currentConstraintIndex, firstDescentConstraintIndex) {
     ...
     */
    static applyPathValuesToSmoothedConstraints(verticalPlan, targetConstraintIndex, currentConstraintIndex, nextMaxAltitude) {
