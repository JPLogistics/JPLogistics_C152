import { ConsumerSubject, KeyInterceptManager } from '../data';
/**
...
export class ThrottleLeverManager {
     ...
     */
    constructor(bus, onInitCallback) {
        this.bus = bus;
        this.throttleLeverRawPositions = Array.from({ length: ThrottleLeverManager.THROTTLE_COUNT }, () => 0);
     ...
     * set.
     */
    setRawThrottleLeverPosition(rawPosition, index) {
        rawPosition = Utils.Clamp(rawPosition, ThrottleLeverManager.RAW_MIN, ThrottleLeverManager.RAW_MAX);
        for (let i = index !== null && index !== void 0 ? index : 1; i < (index !== null && index !== void 0 ? index : ThrottleLeverManager.THROTTLE_COUNT) + 1; i++) {
     ...
     * all throttle levers.
     */
    publishThrottleLeverPosition(position, index) {
        for (let i = index !== null && index !== void 0 ? index : 1; i < (index !== null && index !== void 0 ? index : ThrottleLeverManager.THROTTLE_COUNT) + 1; i++) {
...
ThrottleLeverManager.RAW_MIN = -16384;
ThrottleLeverManager.RAW_MAX = 16384;
ThrottleLeverManager.RAW_RANGE = ThrottleLeverManager.RAW_MAX - ThrottleLeverManager.RAW_MIN;
