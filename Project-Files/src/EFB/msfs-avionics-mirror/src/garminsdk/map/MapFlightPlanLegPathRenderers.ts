 ...
 */
export class MapFlightPlanDirectToCourseLegPathRenderer {
  ...
  public render(
    ...
    const dtoFix = MapFlightPlanDirectToCourseLegPathRenderer.geoPointCache[0].set(leg.calculated.endLat, leg.calculated.endLon);
    const dtoCourseTrue = MagVar.magneticToTrue(leg.leg.course, dtoFix.lat, dtoFix.lon);
    const dtoPath = MapFlightPlanDirectToCourseLegPathRenderer.geoCircleCache[0].setAsGreatCircle(dtoFix, dtoCourseTrue);
    const start = dtoPath.offsetDistanceAlong(dtoFix, UnitType.NMILE.convertTo(-500, UnitType.GA_RADIAN), MapFlightPlanDirectToCourseLegPathRenderer.geoPointCache[1]);

    this.lineRenderer.render(dtoPath, start.lat, start.lon, dtoFix.lat, dtoFix.lon, projection, context, stream, width, style, dash);
  }
}
